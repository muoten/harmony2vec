...
def get_mbid_by_title(title):
	mbid = None
	response = requests.get(f"https://musicbrainz.org/ws/2/work/?query=work:{title}&fmt=json")
...
def get_id_by_youtube_url(youtube_url):
...
def get_available_videos_by_mbid(mbid):
	response = requests.get(f"https://musicbrainz.org/ws/2/work/?query=mbid:{mbid}&fmt=json")
	print(response.json())
...
def get_work_id_by_title(title):
	response = requests.get(f"https://musicbrainz.org/ws/2/work/?query=title:{title}&fmt=json")
	return response.json()['works'][0]['id']


def get_work_id_by_mbid(mbid):
	response = requests.get(f"https://musicbrainz.org/ws/2/recording/{mbid}?fmt=json&inc=work-rels")
...
def get_metadata_by_title(title):
	...
	youtube_id, artist_name, title = get_metadata_by_mbid(recording_id)
	return youtube_id, artist_name, title


def get_recording_id_by_url(url):
	mylinks = []
	response = requests.get(url)
...
def get_metadata_cover_by_work_id_and_mbid_original(work_id, mbid_original):
	...
	if len(array_mbid_original) > 0:
		...
		for i, item_mbid_other in enumerate(array_mbid_other):
			...
			youtube_id, artist_name, title = get_metadata_by_mbid(mbid_other)
...
def get_mbid_by_youtube_id(youtube_id):
	youtube_url = f"https://www.youtube.com/watch?v={youtube_id}"
	try:
		url_id_original = get_id_by_youtube_url(youtube_url)
		print("url_id_original:", url_id_original)
		my_url = f"https://musicbrainz.org/url/{url_id_original}"
		mbid_original = get_recording_id_by_url(my_url)
